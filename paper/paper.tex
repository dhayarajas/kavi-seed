...
\usepackage{url}
\usepackage{float}
\usepackage{siunitx}  % For proper SI units formatting
\usepackage[version=4]{mhchem}  % For chemical formulas

% Hyperref must be loaded LAST (best practice)
\usepackage[pdftex,
            pdfauthor={Anonymous Authors},
            pdftitle={AIRS-GSeed: AI-Driven Groundnut Seed Health Assessment},
            pdfsubject={Remote Sensing, Machine Learning, Agriculture},
            pdfkeywords={groundnut, aflatoxin, UAV, hyperspectral, machine learning},
            pdfproducer={LaTeX with IEEEtran},
            pdfcreator={pdflatex}]{hyperref}

% Configure hyperref colors for print-friendly output
\hypersetup{
    colorlinks=true,
    linkcolor=black,
    citecolor=black,
...
% Mathematical operators in roman (best practice)
\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\argmin}{arg\,min}
\DeclareMathOperator{\softmax}{softmax}
\DeclareMathOperator{\sigmoid}{sigmoid}

% Consistent formatting for model names
...
% Common abbreviations with proper spacing
...
\hyphenation{op-tical net-works semi-conduc-tor ground-nut aflatoxin hyper-spectral multi-modal}
...
UAV flights are conducted weekly during critical growth stages (\SIrange{60}{120}{days} after sowing, DAS) with:
\begin{itemize}
    \item RGB imaging: 4K resolution, \SIrange{1}{3}{\centi\meter} GSD
    \item Multispectral: 5 bands (RGB, Red-edge \SI{730}{\nano\meter}, NIR \SI{850}{\nano\meter}), \SIrange{5}{10}{\centi\meter} GSD
    \item Thermal: $640 \times 512$ resolution, \SIrange{10}{20}{\centi\meter} GSD
\end{itemize}

\subsubsection{Soil \& Pod-Zone Sensors}
Capacitive soil moisture sensors (\SIrange{0}{100}{\percent} VWC), temperature sensors (\SIrange{-40}{85}{\celsius}), and EC sensors are deployed at \SIrange{5}{10}{\centi\meter} depth (pod zone) with \SI{15}{\minute} logging intervals.

\subsubsection{Hyperspectral Seed Analysis}
Portable hyperspectral spectrometers (\SIrange{400}{2500}{\nano\meter}, \SI{3}{\nano\meter} resolution) are used for seed sample analysis, triggered by UAV stress alerts or harvest sampling.

\subsubsection{Storage IoT Sensors}
Continuous monitoring of temperature, relative humidity (RH), \ce{CO2}, and volatile organic compounds (VOC) in storage facilities with \SI{5}{\minute} intervals.
...
The hybrid CNN-ViT model processes multi-modal image inputs $\mathbf{I} = \{\mathbf{I}_{\text{RGB}}, \mathbf{I}_{\text{MS}}, \mathbf{I}_{\text{Thermal}}\}$ to predict stress probability:
%
\begin{equation}
P(y|\mathbf{I}) = \mathrm{Softmax}\left(\mathrm{Concat}[\mathbf{f}_{\text{CNN}}, \mathbf{f}_{\text{ViT}}]\right),
\label{eq:cnn_vit}
\end{equation}
%
where $\mathbf{f}_{\text{CNN}} = \mathrm{ResNet}(\mathbf{I})$ extracts local features and $\mathbf{f}_{\text{ViT}} = \mathrm{Transformer}(\mathrm{PatchEmbed}(\mathbf{I}))$ captures global context. The attention mechanism in ViT is:
%
\begin{equation}
\mathrm{Attention}(\mathbf{Q}, \mathbf{K}, \mathbf{V}) = \softmax\left(\frac{\mathbf{Q}\mathbf{K}^T}{\sqrt{d_k}}\right)\mathbf{V},
\label{eq:attention}
\end{equation}
%
...
%
\begin{equation}
\theta_p = f_{\text{PINN}}(\mathbf{f}_c, \mathbf{s}, \mathbf{w}; \boldsymbol{\theta}),
\label{eq:pinn}
\end{equation}
%
where $\boldsymbol{\theta}$ represents the network parameters. The physics loss incorporates soil moisture diffusion:
%
\begin{equation}
\mathcal{L}_{\text{physics}} = \lambda_1 \left\|\frac{\partial \theta_p}{\partial t} - D\nabla^2 \theta_p\right\|^2 + \lambda_2 \|\theta_p - \theta_{\text{soil}}\|^2,
\label{eq:physics_loss}
\end{equation}
%
where $D$ is the soil moisture diffusivity, $\theta_{\text{soil}}$ is measured soil moisture, and $\lambda_1, \lambda_2$ are weighting factors. The total loss is:
%
\begin{equation}
...
%
\begin{equation}
\text{\shi} = g\left(\mathrm{Attention}\left[\mathbf{h}_{\text{spec}}, \mathbf{h}_{\text{UAV}}, \mathbf{h}_{\text{env}}\right]\right),
\label{eq:shi}
\end{equation}
%
where $\mathbf{h}_{\text{spec}} = \mathrm{CNN1D}(\mathbf{s}_{\text{hyperspectral}})$ encodes hyperspectral features, $\mathbf{h}_{\text{UAV}} = \mathrm{MLP}(\mathbf{f}_{\text{UAV}})$ encodes UAV features, and $\mathbf{h}_{\text{env}} = \mathrm{MLP}(\mathbf{f}_{\text{env}})$ encodes environmental features. The attention mechanism weights each modality:
%
\begin{equation}
...
\end{equation}
%
where $\mathbf{W}_i$ are learnable weight matrices.
...
%
\begin{equation}
\text{\ars} = w_1 \text{\ars}_{\text{spec}} + w_2 \text{\ars}_{\text{field}} + w_3 \text{\ars}_{\text{storage}},
\label{eq:ars}
\end{equation}
%
...
\end{equation}
%
and $\sigma_i$ represents the prediction uncertainty of the $i$-th model. The field component integrates temporal features:
%
\begin{equation}
\text{\ars}_{\text{field}} = \mathrm{LSTM}\left(\mathrm{Transformer}\left[\mathbf{f}_t\right]_{t=1}^{T}\right),
\label{eq:ars_field}
\end{equation}
%
...
%
\begin{equation}
P(y_{t+\Delta t}|\mathbf{X}_{1:t}) = \mathrm{LSTM}\left(\mathrm{Transformer}(\mathbf{X}_{1:t})\right),
\label{eq:temporal}
\end{equation}
%
where $\mathbf{X}_{1:t} = [\mathbf{x}_1, \mathbf{x}_2, \ldots, \mathbf{x}_t]$ is the time series of features up to time $t$, and $\Delta t$ is the prediction horizon (\SIrange{1}{4}{weeks}).
...
Table~\ref{tab:canopy_performance} presents the canopy stress detection performance. \airsgseed\ achieves \SI{89.2}{\percent} accuracy with macro F1-score of 0.87, significantly outperforming baseline methods. The multi-modal approach (RGB + multispectral + thermal) provides \SI{8.5}{\percent} accuracy improvement over RGB-only CNN.
...
\textbf{Method} & \textbf{Accuracy} & \textbf{Macro F1} & \textbf{AUC-ROC} & \textbf{Lead Time} \\
 & (\si{\percent}) & & & (days) \\
\midrule
AIRS-GSeed (Full) & \textbf{89.2} & \textbf{0.87} & \textbf{0.93} & \textbf{10.5} \\
...
The physics-informed neural network achieves pod moisture estimation with RMSE of \SI{6.2}{\percent} VWC and correlation of 0.78 with limited ground truth data. Table~\ref{tab:pod_zone} compares pod-zone inference methods.
...
\textbf{Method} & \textbf{Pod Moisture RMSE} & \textbf{Correlation} \\
 & (\si{\percent} VWC) & \\
\midrule
AIRS-GSeed (PINN) & \textbf{6.2} & \textbf{0.78} \\
...
Table~\ref{tab:shi_performance} shows \shi\ prediction performance. The multi-modal fusion approach achieves $R^2$ of 0.81 and RMSE of 9.8~points (on 0--100 scale), with strong correlation (0.86) to germination rate.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{$R^2$} & \textbf{RMSE} & \textbf{MAE} & \textbf{Correlation} \\
\midrule
...
Table~\ref{tab:ars_performance} presents \ars\ prediction results. The end-to-end model achieves $R^2$ of 0.76 with early warning lead time of \SI{16.3}{days} before regulatory threshold breach.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{$R^2$} & \textbf{RMSE} & \textbf{Lead Time} & \textbf{Risk Cat. Acc.} \\
 & & & (days) & (\si{\percent}) \\
\midrule
AIRS-GSeed (End-to-End) & \textbf{0.76} & \textbf{11.2} & \textbf{16.3} & \textbf{82.4} \\
...
\begin{tabular}{lcc}
\toprule
\textbf{Component Removed} & \textbf{\shi\ $R^2$ Drop} & \textbf{\ars\ $R^2$ Drop} \\
\midrule
